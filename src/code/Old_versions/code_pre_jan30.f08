...
implicit none
integer, parameter :: n=4
integer k, j, i, counter, eqn(n/2), f_state, kdelta, run, maxrun, flag(2)
double precision w(n), xi(n+2), xf(n+2), ea(n), del_ea, gamma, arg, numt(n/2), avgnumt(n/2), numr(n/2), avgnumr(n/2), denom, heaviside
gamma  = (sqrt(3.0)-1.0)/2.0
del_ea = 2.0*gamma
denom = 0.0
...
do run = 1, maxrun
   call s_crossing(numt,numr,xi,n)
   do i = 1, n/2
...
print *, avgnumt(1)/denom, avgnumt(2)/denom
end subroutine mc_avg

subroutine s_crossing(numt,numr,xi,n)
!==========================================================!
! This subroutine calculates the crossing probabilities of !
! the single and double avoided crossing problems          !
!----------------------------------------------------------!
! Daniel Celis Garza 27 Jan 2015                           !
! wish me luck.                                            !
! delta_n -> del_ea (delta electronic action variable)     !
! n       -> ea     (electronic action variable)           !
! N       -> eqn    (electronic quantum number)            !
...
integer n, flag(2), k, i, i_state, f_state, kdelta, j
integer eqn(n)
double precision ti, tf, dt, rmin, rmax, gamma, del_ea, arg, heaviside, w(n)
double precision xi(n+2), xf(n+2), ea(n), rndnum(n/2), q(n/2)
double precision numt(n/2), numr(n/2), num_dummy
external sc_motion
j = 0
! Parameters
ti    = 0.0   ! Initial time.
dt    = 0.5   ! Time step.
rmin  = -4.0  ! Minimum distance.
...
! Action variables and calculating initial state Window functions.
do k = 1, n/2
   call random_number(rndnum)
   eqn(k) = kdelta(i_state, k)
   ea(k) = eqn(k) + gamma*(2.0*rndnum(1)-1.0)
   q(k)  = 2.0*pi*rndnum(2)
   arg  = del_ea/2.0 - abs(ea(k)-eqn(k))
   w(k) =  heaviside(arg)/del_ea
end do

! Cartesian variable
xi(1) = rmin ! Initial position R.
xi(2) = 30.0  ! Initial momentum P.
xi(3) = sqrt(2*(ea(1)+gamma))*cos(q(1))   ! Initial x1
xi(4) = -sqrt(2*(ea(1)+gamma))*sin(q(1))  ! Initial p1
xi(5) = sqrt(2*(ea(2)+gamma))*cos(q(2))   ! Initial x2
xi(6) = -sqrt(2*(ea(2)+gamma))*sin(q(2))  ! Initial p2

!print *, "1", w(1), w(2) 
!print *, "1", w(3), w(4)
!open(unit=1, file='trajn.dat')
!open(unit=2, file='trajg.dat')
!write(1,*) ti, xi(1), xi(2), xi(3), xi(4), xi(5), xi(6)
!write(2,*) ti, xi(1), xi(2), xi(3), xi(4), xi(5), xi(6)
! Integerate equations of motion
do while(rmin<=rmax)
   tf = ti + dt
   call rk4gn(sc_motion, ti, tf, xi, xf, n+2)
   !write(1,*) tf, xf(1), xf(2), xf(3), xf(4), xf(5), xf(6)
   !call rk4n(sc_motion, ti, tf, xi, xf, n+2)
   !write(2,*) tf, xf(1), xf(2), xf(3), xf(4), xf(5), xf(6)
...
! Calculating nk(t), ea(3) = n1(t), ea(4) = n2(t)
do k = 1, n/2
   ea(n/2+k) = (xf(n/2+k+j)**2 + xf(n/2+k+1+j)**2)/2
j = j + 1
end do

if(1.0 <= ea(3) .and. ea(3) <= 1.0 + del_ea .and. 0 <= ea(4) .and. ea(4) <= del_ea) then
   f_state = 1
else if(0.0 <= ea(3) .and. ea(3) <= del_ea .and. 1.0 <= ea(4) .and. ea(4) <= 1.0 + del_ea) then
   f_state = 2
else
   flag(2) = 1
end if

! Eliminating those which don't fit the selection criteria and calculating all numerators.
! Calculating Nk(t), eqn(3) = N1(t), eqn(4) = N2(t)
! If flag(2) == 1 then numr(i) and numt(i) = 0.0 and the run adds nothing of value.
if(flag(2) /= 1) then
   do k = 1, n/2
      eqn(n/2+k) = kdelta(f_state, k)
      arg  = del_ea/2.0 - abs(ea(n/2+k)-eqn(n/2+k))
      w(n/2+k) =  heaviside(arg)/del_ea
      !print *, f_state, k, eqn(n/2+k), arg, w(n/2+k)
   end do
   ! Calculating the numerator for transmission.
   if(flag(1) == 0) then
      do k = 1, n/2
         ! w(k) are initial state window functions, w(n/2+k) are the final state window functions
         num_dummy = num_dummy*w(k)*w(n/2+k)
      end do
      numt(f_state) = num_dummy
      !print *, numt(1), numt(2)
   ! Calculating the numerator for reflection.
   else if(flag(1) == 1) then
      do k = 1, n/2
         num_dummy = num_dummy*w(k)*w(n/2+k)
      end do
      numr(f_state) = num_dummy
      !print *, numr(1), numr(2)
   end if
end if
!print *, "2", w(3), w(4)
