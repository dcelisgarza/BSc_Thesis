...
!write(1,*) "# h = 1/(10ip), mc_steps = 7500, gamma = 0.366"
!write(1,*) "# Initial Momentum  ", "R 1<-1  ", "R 2<-1  ", "T 1<-1  ", "T 2<-1  "
ip = 1.0

!Initial ones.
call mc_avg(avgnumr,avgnumt,denom,ip,n)
...
subroutine mc_avg(avgnumr,avgnumt,denom,ip,n)
implicit none
...
external sc_eq, dc_eq ! Single crossing Hamilton's eqs.
...
!do k = 1, n/2
!   if(1.0 .le. snf(k) .and. snf(k) .le. 1.0 + dsn) then
!      statef = k
!      go to 200
!   end if
!end do
!if(statef .eq. 0) go to 100
!200 continue
...
dx(1) = x(2)/mu
if(x(1) .ge. 0.0) then
   dx(2) = 2.0*C*D*exp(-D*x (1)**2.0)*x(1)*(x(4)*x(6)+x(3)*x(5))+0.5*(-x(4)**2.0+x (6)**2.0-x (3)**2.0+x (5)**2.0)*A*B*exp(-B*x(1))
   dx(3) = C*exp (-D*x (1) ** 2.0)*x (6) + x (4)*(A - A*exp (-B*x (1)))
   dx(4) = -C*exp (-D*x (1) ** 2.0)*x (5) - x (3)*(A - A*exp (-B*x (1)))
   dx(5) = C*exp (-D*x (1) ** 2.0)*x (4) - x (6)*(A - A*exp (-B*x (1)))
   dx(6) = -C*exp (-D*x (1) ** 2.0)*x (3) + x (5)*(A - A*exp (-B*x (1)))
else if(x(1) .lt. 0.0) then
   dx(2) = 2.0*C*D*exp(-D*x (1)**2.0)*x(1)*(x(4)*x(6)+x(3)*x(5))+0.5*(-x(4)**2.0+x (6)**2.0-x (3)**2.0+x (5)**2.0)*A*B*exp(B*x(1))
   dx(3) = C*exp (-D*x (1) ** 2.0)*x (6) + x (4)*(A*(-1 + exp (B*x (1))))
   dx(4) = -C*exp (-D*x (1) ** 2.0)*x (5) - x (3)*(A*(-1 + exp (B*x (1))))
   dx(5) = C*exp (-D*x (1) ** 2.0)*x (4) - x (6)*(A*(-1 + exp (B*x (1))))
   dx(6) = -C*exp (-D*x (1) ** 2.0)*x (3) + x (5)*(A*(-1 + exp (B*x (1))))
end if
!dx(1) = x(2)/mu
!if(x(1) .ge. 0.0) then
!   dx(2) = 2.0*d*c1*x(1)*(x(4)*x(6)+x(3)*x(5))&
!           +0.5*(-x(4)**2.0+x(6)**2.0-x(3)**2.0+x(5)**2.0)*a*b*exp(-b*x(1))
!   dx(3) = c1*x(6)+x(4)*c2
!   dx(4) = -c1*x(5)-x(3)*c2
!   dx(5) = c1*x(4)-x(6)*c2
!   dx(6) = -c1*x(3)+x(5)*c2
...
eo = 0.05
mu = 2000
c1 = 0.5*(a*exp(-b*x(1)**2)-eo)
c2 = c*exp(-d*x(1)**2)

dx(1) = x(2)/mu
dx(2) = 0.5*exp(-(b+d)*x(1)**2)*x(1)*(4*d*c*exp(b*x(1)**2)*(x(4)*x(6)+x(3)*x(5))&
        +b*a*exp(d*x(1)**2)*(-2+x(4)**2-x(6)**2+x(3)**2-x(5)**2))
dx(3) = c1*x(4)+c2*x(6)
dx(4) = -c1*x(3)-c2*x(5)
dx(5) = c2*x(4)-c1*x(6)
dx(6) = -c2*x(3)+c1*x(5)
