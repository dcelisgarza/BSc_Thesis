...
CALL CPU_TIME(start)
open(unit=1, file='sc_prob_lh.dat')
...
CALL CPU_TIME(start)
open(unit=1, file='dc_prob_lh.dat')
write(1,*) "# h = 1/(0.0125*ip), mc_steps = 15000, gamma = 0.366"
...
double precision days, hours, minutes, seconds
CALL CPU_TIME(start)
open(unit=1, file='ec_prob_lh2.dat')
write(1,*) "# h = 1/(0.10125*ip), mc_steps = 15000, gamma = 0.366"
write(1,*) "# Initial Momentum  ", "R 1<-1  ", "R 2<-1  ", "T 1<-1  ", "T 2<-1  "
